...
写完一行注释后问自己：「**删掉这行注释，代码还清晰吗？**」是 → 不写；否 → 写一句 WHY。
...
注释的目的是传达**非显然信息**（WHY），不重复代码本身（WHAT）。下面是高频翻车场景：
...
// ❌ 反例 4：注释比代码长
// 加油时间范围选择器在「选同一天」场景下的默认时间配置：
// Element Plus 的 el-date-picker（type="daterange"）默认 start/end 时间均为 00:00:00，
// 这会导致用户选同一天查询时只能查到该时刻的加油记录（甚至查不到任何数据）。
// 通过 :default-time 指定 start=00:00:00、end=23:59:59，
// 使 picker 在用户选同一天时自动展开为「当天 00:00:00 ~ 当天 23:59:59」。
// 注意：:default-time 仅影响 picker 的默认时间显示，不会改写 v-model 的实际值；
// 实际请求的 end 时间仍需在 queryList 中通过 setHours(23, 59, 59) 显式修正。
const defaultTime: [Date, Date] = [new Date(0, 0, 0, 0, 0, 0), new Date(0, 0, 0, 23, 59, 59)]

// ✅ 正例：2 行讲清 why，代码本身不重复
// 选同一天时 picker end 停在 00:00:00，:default-time 只影响 UI，
// 请求里需 setHours 把 end 显式推到 23:59:59.999
const defaultTime: [Date, Date] = [new Date(0, 0, 0, 0, 0, 0), new Date(0, 0, 0, 23, 59, 59)]
```
