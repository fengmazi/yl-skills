...
> 本页描述引领体系 PC 项目的**通用部署流程**，与前端构建工具（Vite / Vue CLI）无关。
> 构建相关配置（scripts、.env 文件、vite.config.ts / vue.config.js）请查阅各 version skill 的 startup-config.md。

## deploy.js — SSH + SCP 部署

将构建产物 `dist/` 通过 SSH + SCP 部署到测试服务器，包含备份、清理、上传、本地清理完整流程。
